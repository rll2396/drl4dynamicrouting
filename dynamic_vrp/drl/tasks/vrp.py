...
class VehicleRoutingDataset(Dataset):
    ...
    def update_dynamic(self, dynamic, chosen_idx):
        ...
        for batch in all_demands:
            nonzero_indices = batch.nonzero().squeeze()
            if len(nonzero_indices.size()) > 0:
                for index in nonzero_indices:
                    if index != 0:
                        min_change = -1
                        if batch[index] == 1/float(self.max_load):
                            min_change = 0
                        max_change = 2
                        if batch[index] == self.max_demand/float(self.max_load):
                            max_change = 1
                        change = np.random.randint(min_change, max_change)
                        batch[index] += change/float(self.max_load)
