...
def train(actor, critic, task, num_nodes, train_data, valid_data, reward_fn,
    ...
    for epoch in range(20):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            # original_reward = reward_fn(static,tour_indices)
            # print('The original reward is:', original_reward)
...
def train_tsp(args):
    ...
    print('Create train_data and valid_data successfully')
...
def train_vrp(args):
    ...
    critic = StateCritic(STATIC_SIZE, DYNAMIC_SIZE, args.hidden_size).to(device)
    print('Create train_data, validate_data, actor, critic successfully ')
    ...
    if not args.test:
        print("Now we begin to train the data")
        train(actor, critic, **kwargs)

    print("Now we create test data")
...
if __name__ == '__main__':
    ...
    # args.checkpoint = os.path.join('tsp', '20', '11_25_45.413686' + os.path.sep)
    ...
    test_dir = 'test'
    test_loader = DataLoader(test_data, args.batch_size, False, num_workers=0)
    out = validate(test_loader, actor, tsp.reward, tsp.render, tsp.render_static, test_dir, num_plot=0)
